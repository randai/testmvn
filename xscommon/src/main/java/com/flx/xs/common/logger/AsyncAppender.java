package com.flx.xs.common.logger;
...
/**
 * This {@link Appender} logs {@link ILoggingEvent}s asynchronously. It acts
 * solely as an event dispatcher and must therefore be attached to one or more
 * child appenders in order to do useful logging. It is the user's
 * responsibility to close appenders, typically at the end of the application
 * lifecycle.
 * <p>
 * The appender buffers events in a {@link BlockingQueue} on the application
 * thread(s). The appenders {@link Dispatcher} thread takes events from the head
 * of the queue, and dispatches them to all the appenders that are attached to
 * this appender. With the method {@link #getQueueSize()} the number of events
 * currently stored in the event queue can be retrieved.
 * <p>
 * The appenders event queue is configured with a maximum capacity of
 * {@link #DEFAULT_QUEUE_CAPACITY 1000} (defined by the variable
 * {@link #setQueueCapacity(int) QueueCapacity}). If the queue is filled up,
 * then application threads are blocked from logging new events until the
 * dispatcher thread has had a chance to dispatch one or more events. When the
 * queue is no longer at its maximum configured capacity, application threads
 * are able to start logging events once more. Asynchronous logging therefore
 * becomes pseudo-synchronous when the appender is operating at or near the
 * capacity of its event buffer. This is not necessarily a bad thing. It's the
 * price a threaded application will have to pay sometimes. The appender is
 * designed to allow the application to keep on running, albeit taking slightly
 * more time to log events until the pressure on the appenders buffer eases.
 * <p>
 * Optimally tuning the size of the appenders event queue for maximum
 * application throughput depends upon several factors. Any or all of the
 * following factors are likely to cause pseudo-synchronous behavior to be
 * exhibited:
 * <ul>
 ...
 * </ul>
 * To keep things moving, increasing the size of the appenders queue will
 * generally help, at the expense of heap available to the application when
 * large numbers of logging events are queued.
 * <p>
 * It's possible to attach separate instances of this appender to one another to
 * achieve multi-threaded dispatch. For example, assume a requirement for two
 ...
 * appenders attached, B and C. Let's say B in turn has a child file I/O
 * appender attached, and C has a child messaging appender attached. This will
 * result in fast dispatch from A to both B and C via As dispatch thread. Bs
 * dispatch thread will be dedicated to logging to file I/O, and Cs dispatch
 * thread will be dedicated to logging via messaging.
 * <p>
 ...
 * <a href="http://logback.qos.ch/manual/mdc.html">MDC</a> are copied. To
 * configure the appender to copy the caller data as well, set the variable
 * {@link #setIncludeCallerData(boolean) IncludeCallerData} to <code>true</code>.
 * <p>
...
public class AsyncAppender<E> extends UnsynchronizedAppenderBase<E> implements
   ...
   * 
   * <p>
   * The <code>QueueCapacity</code> variable is set to <code>1000</code> by
   * default.
   */
   ...
   * application thread.
   * <p>
   * <b>This operation is expensive (a stack trace is created). So don't use it
   * if the caller data is not logged.</b>
   * 
   * <p>
   * The <code>IncludeCallerData</code> variable is set to <code>false</code> by
   * default.
   */
  ...
  /**
   * {@link Runnable} which forwards {@link LoggingEvent}s to the attached
   * appenders
   */
   ...
   * 
   * @param queueCapacity
   *          must be within the range of 1 and {@link Integer.MAX_VALUE}
   */
  ...
  /**
   * Specifies if an events caller data should be calculated before the event is
   * added to the event queue. The default value is <code>false</code>.
   * 
   * @param includeCallerData
   *          if <code>true</code> then caller data is calculated
   */
   ...
   * 
   * @return current queue size or <code>-1</code> if the appender is not
   *         started
   */
  ...
  public void start() {
    if (!super.isStarted()) {
      ...
      dispatcherThread.setName(DISPATCHER_THREAD_NAME_PREFIX + " [" + getName()
  ...
  public void stop() {
    if (super.isStarted()) {
      ...
      try {
        ...
        addError("We're interrupted while waiting for the "
  ...
  /**
   * Used by thread to retrieve {@link LoggingEvent}s from a
   * {@link BlockingQueue} in a loop and forward them to attached
  ...
  private static class Dispatcher<E> implements Runnable {
    ...
    private Dispatcher(Appender<E> asyncAppender,
        AppenderAttachableImpl<E> appenders, BlockingQueue<E> queue) {
